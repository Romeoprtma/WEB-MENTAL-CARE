...
@section('tambahMeditasi')

<h2 class="text-3xl font-extrabold text-indigo-700 mb-4 text-center tracking-wide mt-4">Tambah Meditasi</h2>
<form action="{{ route('kelolaMeditasiPsikolog.store') }}" method="POST" enctype="multipart/form-data" class="max-w-lg mx-auto bg-gradient-to-br from-blue-50 to-indigo-100 p-8 rounded-2xl shadow-lg border border-indigo-200">
    @csrf
    <div class="mb-5">
        <label for="title" class="block text-indigo-700 font-semibold mb-1">Judul Meditasi</label>
        <input type="text" name="title" id="title" class="w-full border-2 border-indigo-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-400 transition" required>
    </div>

    <div class="mb-5">
        <label for="description" class="block text-indigo-700 font-semibold mb-1">Deskripsi</label>
        <textarea name="description" id="description" rows="4" class="w-full border-2 border-indigo-200 rounded-lg px-4 py-2 focus:outline-none focus:border-indigo-400 transition resize-none" required></textarea>
    </div>
    <div class="mb-5">
